...
bool declares_seed(const std::string& strategy)
{
    ...
    return std::any_of(info->schema.begin(), info->schema.end(), [](const ParamSpec& spec) { return spec.name == "seed"; });
}
...
Strategies resolve_seeded_strategies(const py::sequence& specs, const py::object& seed)
{
    ...
    for (const py::handle& handle : specs)
    {
        ...
        Params params;
        if (!seed.is_none() && py::isinstance<py::str>(spec) && declares_seed(spec.cast<std::string>()))
        {
            params.emplace("seed", ParamValue(seed.cast<int64_t>() + seat));
        }
...
void bind_match(py::module_& module)
{
    py::class_<PyMatch>(module, "Match", "One game between strategies, stepped by hand or played to the end.")
        ...
        .def("state", [](PyMatch& match) { return create_shared<PyState>(match.match().state()); }, py::keep_alive<0, 1>())
        ...
        .def("apply", [](PyMatch& match, ActionId action)
            {
                require_legal(match.match().state(), action);
