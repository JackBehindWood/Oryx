...
from build_system.deps.resolve import (
    DependencyError,
    ResolvedDependency,
    ensure,
    fetch,
...
def _confirm_fetch(deps: list[ResolvedDependency]) -> bool:
    from build_system.interactive import is_interactive
...
def add(
    ...
    if submodule and not dep.present:
        from build_system.deps.sources.submodule import SubmoduleSource
...
def update(
    ...
    run: RunContext = ctx.obj
    dep = _find(run, name)
    from build_system.deps.sources import source_for
