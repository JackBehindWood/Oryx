...
def test_tasks_fresh(tmp_path):
    data = _read(tasks.write_tasks(path=tmp_path / "tasks.json"))
...
def test_tasks_keep_user_entries(tmp_path):
    ...
    data = _read(tasks.write_tasks(path=path))
...
def test_launch_entries_per_profile(tmp_project):
    ...
    data = _read(launch.write_launch(BuildConfig(), debugger="lldb", path=path))
...
def test_launch_cppdbg_keys(tmp_project, monkeypatch, system, mode, tool):
    ...
    data = _read(launch.write_launch(BuildConfig(), debugger="cppdbg", path=tmp_project / "launch.json"))
...
def test_settings_merge_keeps_user_keys(tmp_project):
    ...
    data = _read(settings.write_settings(path=path))
...
def test_c_cpp_properties_linux(tmp_project, fake_python, monkeypatch):
    ...
    data = _read(c_cpp_properties.write_c_cpp_properties(BuildConfig(), path=path))
    ...
    assert linux["includePath"] == [
        ...
        (tmp_project / "build" / "generated").as_posix(),
...
def test_c_cpp_properties_defines_without_python(tmp_project, profile, defines):
    data = _read(c_cpp_properties.write_c_cpp_properties(BuildConfig(profile=profile, python_enabled=False), path=tmp_project / "c.json"))
...
def test_c_cpp_properties_macos_has_two_configurations(tmp_project, monkeypatch):
    ...
    data = _read(c_cpp_properties.write_c_cpp_properties(BuildConfig(python_enabled=False), path=tmp_project / "c.json"))
