...
from pyforge.setup import stale_objects
...
def test_first_configure_without_outputs_wipes_nothing(tmp_project):
    assert stale_objects.wipe_outputs_if_options_changed(tmp_project / "build", None, "a") is False
    assert _outputs(tmp_project) == (False, False)


@pytest.mark.parametrize("previous", [None, ""])
def test_unknown_previous_options_wipe_existing_outputs(tmp_project, previous):
    _make_outputs(tmp_project)
    assert stale_objects.wipe_outputs_if_options_changed(tmp_project / "build", previous, "a") is True
    assert _outputs(tmp_project) == (False, False)


def test_unchanged_options_keep_outputs(tmp_project):
    _make_outputs(tmp_project)
    assert stale_objects.wipe_outputs_if_options_changed(tmp_project / "build", "a", "a") is False
    assert _outputs(tmp_project) == (True, True)


def test_changed_options_wipe_outputs(tmp_project):
    _make_outputs(tmp_project)
    assert stale_objects.wipe_outputs_if_options_changed(tmp_project / "build", "a", "b") is True
    assert _outputs(tmp_project) == (False, False)


def test_legacy_options_file_is_removed(tmp_project):
    legacy = tmp_project / "build" / ".python-options"
    legacy.write_text("--no-python", encoding="utf-8")
    stale_objects.wipe_outputs_if_options_changed(tmp_project / "build", "a", "a")
...
def test_prune_clears_only_projects_with_missing_prerequisites(tmp_project, project, monkeypatch):
    ...
    def fake_run(command, cwd=None, **kwargs):
        ...
        if "Tests.make" in command:
            raise subprocess.CalledProcessError(2, command, stderr="make: *** No rule to make target '../tests/old.cpp', needed by 'x.o'.")
...
def test_prune_ignores_a_missing_library_from_another_project(tmp_project, project, monkeypatch):
    def fake_run(command, cwd=None, **kwargs):
        raise subprocess.CalledProcessError(
            2, command, stderr="make: *** No rule to make target 'bin/Debug-linux-x86_64/Oryx/libOryx.a', needed by 'bin/Debug-linux-x86_64/Tests/Tests'."
        )

    monkeypatch.setattr(stale_objects, "run_command", fake_run)
    objects = tmp_project / "build" / "bin-int" / "Debug-linux-x86_64" / "Tests"
