...
from pyforge import compile_commands
...
def test_generate_without_make(tmp_project, monkeypatch):
    ...
    assert compile_commands.generate_compile_commands("debug_x64", tmp_project / "build") is None
...
def test_generate_writes_every_project(tmp_project, make_calls, monkeypatch):
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/bin/make")
    path = compile_commands.generate_compile_commands("release_x64", tmp_project / "build")
