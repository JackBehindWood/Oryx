...
| Python build and hosts | Python is on by default and opt-out at build time (`--no-python` / `python = false`); only `Oryx/backends/Python/` and the `OryxPy` project need pybind11 and `<Python.h>`, and CI gets a Python-off leg. Both hosts are supported (`import oryx`, and Oasis embedding an interpreter) from one binding source; pybind11 is built by Premake and vendored at `Oryx/vendor/pybind11`. Details: [Python API](python-api.md#hosts-and-build) | Working decision |
...
| Python scope choices | TicTacToe and its heuristic stay in `Oasis` (`OryxPy` compiles them directly); numpy is optional (`oryx[numpy]`); `oryx.log` and `oryx.assertions` are Tier 1 modules | Working decision |
| Python config, seeding, extension module | Config keys `[python] enabled` (build option) and `[scripting] paths` (run-time). `simulate(seed=S)` gives strategies created by name that declare a `seed` parameter `S + seat_index`, with no `IStrategy`/`BatchRunner` change. `OryxPy` is the small Premake project producing the `_oryx` shared library, skipped when Python is off | Working decision (layout open until the build integration) |
...
| Initialisation | `oryx::init()` (`Application.h`) initialises logging and is idempotent; hosts call it before anything logs: `EntryPoint.h`'s `main()`, the test `main()`, and `oryx.init()` in notebooks and the REPL. `oryx::is_initialised()` reads a global flag. There is no standard-error fallback in `Error::log()`. Python guards only `make_game` and `make_strategy` so far; the guard raises, it does not lazy-init. See [Python API](python-api.md) | Working decision |
