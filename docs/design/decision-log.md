...
| Script reload | `ReloadScriptsEvent` makes `ScriptingLayer` re-discover, `unload()` each running runtime and `reload()` every source; Oasis has no trigger yet. Helper modules are not re-imported. See [Python API](python-api.md) | Working decision |
| Embedded `sys.path` | The venv's site-packages (baked from `sysconfig`) and the project root (working directory) are appended to the embedded interpreter's `sys.path` | Working decision |
