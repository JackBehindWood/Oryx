...
* **Logging is always initialised.** The host calls `oryx::init()` before any layer runs (`EntryPoint.h`'s `main()`, the test `main()`, and `oryx.init()` from Python); it is idempotent and `oryx::is_initialised()` reports the state. There is no standard-error fallback, so code that logs may assume the loggers exist.
